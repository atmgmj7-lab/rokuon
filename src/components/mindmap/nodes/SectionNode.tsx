...
export default function SectionNode({ id, data, selected }: NodeProps<SectionNodeData>) {
  ...
  return (
    <div
      className={`rounded-2xl border-2 border-dashed flex flex-col items-start justify-start p-3 transition-shadow ${selected ? "shadow-md" : ""}`}
      style={{ borderColor: color, backgroundColor: `${color}14`, minWidth: 200, minHeight: 120, width: "100%", height: "100%" }}
    >
      <NodeResizer
      ...
      <div className="flex items-center gap-1 w-full">
        <Shapes className="w-4 h-4 shrink-0" style={{ color }} />
        {editing ? (
          <input
            autoFocus
            value={label}
            ...
            className="nodrag flex-1 text-sm font-bold border-b outline-none bg-transparent"
            style={{ color, borderColor: color }}
          />
        ) : (
          <span
            className="nodrag flex-1 text-sm font-bold cursor-text select-none"
            style={{ color }}
